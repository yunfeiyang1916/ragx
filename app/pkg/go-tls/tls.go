...
func SetLogInfo(info *LogInfo) {
	Set(logInfoKeyActive, info)
}

func SetAnyLogInfo(info interface{}) {
	Set(logInfoKeyActive, info)
}

func GetLogInfo() (*LogInfo, bool) {
	info, ok := Get(logInfoKeyActive)
	if !ok {
		return nil, false
	}
	return info.(*LogInfo), ok
}
